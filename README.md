...
> да, то определить тип треугольника: равносторонний, равнобедренный, разносторонний или прямоугольный. Треугольник может быть разных типов одновременно.
...
- Для 2 подзадачи необходимо рассмотреть 4 случая:
    ...
    4. `A*A + B*B == C*C || B*B + C*C == A*A || A*A + C*C == B*B` прямоугольный треугольник
...
На выход программа выдаёт строку. Всего вариантов таких строк 5, а именно:
...
Пусть A, B и C - произвольные стороны треугольника. Тогда:
1. A+B>C и A+C>B и B+C>A условие существования любого треугольника - сумма двух любых сторон должна превосходить третью.
2. A=B и B=C равносторонний треугольник
3. A=B и A≠C или A=C и B≠C или B=C и A≠C  равнобедренный треугольник
4. $A^2$ + $B^2$ == $C^2$ или $A^2$ + $C^2$ == $B^2$ или $B^2$ + $C^2$ == $A^2$ прямоугольный треугольник
5. A≠B и B≠C и A≠C разносторонний треугольник

Так как работа происходит в натуральных числах, ситуация равнобедренного прямоугольного треугольника не существует. Поэтому каждый получаемый программой прямоугольный треугольник по умолчанию будет являться разносторонним.
...
Выводом программы будет строка. Для вывода результата необязательно его хранить в отдельной переменной.
...
3. **Проверка типа треугольника, если таковой существует:**
    - Если переменные a, b, и c совпадают по значениям, то треугольник является равносторонним, о чем программа сообщает пользователю и завершает свою работу. Иначе программа продолжает рассматривать следующий случай.
    - Если a=b или a=c или b=c, то треугольник является равнобедренным, о чем программа сообщает пользователю и завершает свою работу. Иначе программа продолжает рассматривать следующий случай.
    - Если $a^2$ + $b^2$ = $c^2$ или $a^2$ + $c^2$ = $b^2$ или $b^2$ + $c^2$ = $a^2$, то треугольник является прямоугольным, о чем программа сообщает пользователю и завершает свою работу.
