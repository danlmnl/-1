...
3. **Проверка типа треугольника, если таковой существует:**
    - Если переменные `a, b, c` совпадают по значениям, то треугольник является равносторонним, о чем программа сообщает пользователю и завершает свою работу. Иначе программа продолжает рассматривать следующий случай.
    - Если `a == b || a == c || c == b`, то треугольник является равнобедренным, о чем программа сообщает пользователю и завершает свою работу. Иначе программа продолжает рассматривать следующий случай.
